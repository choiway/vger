...
## FUNCTIONS
getNorgate <- function(X){ 
	if (length(grep("\\$",X))==1){ 
	t <-	read.csv(paste('~/Documents/data_norgate/Cash/',X,'.csv',sep=""),header=F)
	} else {  
	t <-	read.csv(paste('~/Documents/data_norgate/Continuous Futures/Back Adjusted/',X,'.csv',sep=""),header=F)
	}	
	colnames(t) <- c('dt', 'open', 'high', 'low', 'close', 'volume')
	t <- xts(t[,2:6],as.Date(t[,1],"%m/%d/%y"))
	return(t)
}
